...
const _TokenAccountRow: React.FC<TokenAccountRowProps> = ({ token, onReclaim }) => (
  <div
  ...
  >
    <div className="flex items-center">
      <p className="truncate" title={token.mint}>
        {token.mint}
      </p>
    </div>
    ...
    <div className="flex justify-center items-center">
      <Button onClick={() => onReclaim(token)} variant="secondary" size="sm" className="font-light">
        Reclaim Rent
      </Button>
    </div>
  </div>
...
const ReclaimContent = () => {
  ...
  const _handleReclaim = (token: TokenAccount) => {

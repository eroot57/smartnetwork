...
import type { AIResponse } from '@/lib/ai/agent';
import type { WalletState } from '@/types/wallet';
import { AlertCircle, Bot, CheckCircle, Send, XCircle } from 'lucide-react';
...
export function AIAssistant({ walletState, onTransactionAdvice }: AIAssistantProps) {
  ...
  const { isThinking, askAI, error } = useAIAgent(walletState);
  ...
  const _handleAIResponse = (response: AIResponse) => {
  ...
  const renderMessage = (message: Message) => {
    ...
    return (
      ...
      >
        ...
        >
          {isAssistant && <Bot className="w-6 h-6 mr-2 flex-shrink-0 text-blue-600" />}
          <div className="flex flex-col">
            ...
            {message.status && (
              <div className="flex items-center mt-2">
                ...
                {message.status === 'error' && <XCircle className="w-4 h-4 text-red-500 mr-1" />}
              </div>
            )}
          </div>
        </div>

...
const ReclaimForm = ({ tokenAccount, onSubmit }: Props) => {
  ...
  const handleReclaim = async () => {
    try {
      ...
      const response = await WalletAI.evaluateTransaction(
        tokenAccount.mint,
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-gray-400 font-light text-xs pb-0 leading-[15px] italic pt-1">
        This will reclaim all the tokens in the ATA. After reclaiming, the original ATA will be
        closed and the rent will be reclaimed.
      </p>

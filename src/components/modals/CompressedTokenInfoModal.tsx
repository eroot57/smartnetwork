...
const CompressedMintInfoModal = ({ open, onClose, token }: Props) => {
  const { isFetchingMintInfo, errorFetchingMintInfo, mintInfo, isAuthority } = useMintInfo(
    token?.mint
  );
            ...
            <div className="pt-3">
              <div className="text-sm text-gray-500 flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <p>{mintInfo?.decimals}</p>
                </div>
                <div className="flex items-center gap-2">
                  <p className="text-gray-700">Authority:</p>
                  <p>{isAuthority ? 'You' : mintInfo?.mintAuthority?.toBase58() || ''}</p>
                </div>
                <div className="flex items-center gap-2">
                  <p className="text-gray-700">Balance:</p>
                  <p>{formatAmount(token?.balance, mintInfo?.decimals)}</p>
                </div>
              </div>
            </div>
          </>
